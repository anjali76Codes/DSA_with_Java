package DSA_with_Java.Queue_Exercise;
...
class Tester {
    ...
    public static Queue[] splitQueue(Queue queue) {
        ...
        while(! queue.isEmpty()){
            int delElement = queue.dequeue();
            if(delElement %2 == 0 ){
                evenQueue.enqueue(delElement);
                
            }
            
            else {
                oddQueue.enqueue(delElement);
            }
        }
        
       Queue[] newArray = new Queue[2];
       
      newArray[0] = evenQueue;
      newArray[1] = oddQueue;
